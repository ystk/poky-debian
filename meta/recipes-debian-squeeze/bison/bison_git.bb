...
# Exclude "doc" and "examples" sub directories to avoid compile errors
# related to "bison.texinfo" and "fdl.texi"
do_unpack_srcpkg_append() {
	sed -i "s|doc/Makefile|])|" ${S}/configure.ac
	sed -i "s|doc/yacc.1])||" ${S}/configure.ac
	sed -i -e "s/ doc//" -e "s/ examples//" ${S}/Makefile.am
}

# git fetch: Avoid re-generating .c because
# yacc is provided by bison-native itself.
