...
DEBIAN_SQUEEZE_SRCPKG_NAME = "boost1.42"
SRC_URI = " \
file://arm-intrinsics.patch \
"
...
do_install_append() {
	for lib in ${BOOST_LIBS}; do
	    if [ -e ${D}${libdir}/libboost_${lib}.a ]; then
		ln -s libboost_${lib}.a ${D}${libdir}/libboost_${lib}-mta.a
	    fi
	    if [ -e ${D}${libdir}/libboost_${lib}.so ]; then
		ln -s libboost_${lib}.so ${D}${libdir}/libboost_${lib}-mta.so
	    fi
	done
}

SRC_URI += " \
	file://libstdcpp3.hpp.patch \
	file://boost-1.48.0-gcc47-winthreads.patch \
	"
