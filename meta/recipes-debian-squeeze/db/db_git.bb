...
DEBIAN_SQUEEZE_SRCPKG_NAME = "db4.8"

LIC_FILES_CHKSUM = "file://../LICENSE;md5=1e17379fdf7e95628f4bc9f649d504a9"

PR = "r0"

S = "${WORKDIR}/db4.8-${PV}/dist"

# Use "${TARGET_SYS}-nm" (cross binutils) under sysroots instead of "nm".
# Native nm returns "File format not recognized" when 64bit objects
# are given as arguments. This line is added by Debian patch, so
# cross build may be not supported.
do_patch_append() {
...
replace_nm() {
	sed -i "s@^\tnm @\t${TARGET_SYS}-nm @" ${S}/Makefile.in
}

do_install_append() {
	mkdir -p ${D}/${includedir}/db48
	#mv ${D}/${includedir}/db_185.h ${D}/${includedir}/db48/.
	mv ${D}/${includedir}/db.h ${D}/${includedir}/db48/.
	mv ${D}/${includedir}/db_cxx.h ${D}/${includedir}/db48/.
	#mv ${D}/${includedir}/dbsql.h ${D}/${includedir}/db48/.
	#ln -s db48/db_185.h ${D}/${includedir}/db_185.h
	ln -s db48/db.h ${D}/${includedir}/db.h
	ln -s db48/db_cxx.h ${D}/${includedir}/db_cxx.h
	#ln -s db48/dbsql.h ${D}/${includedir}/dbsql.h
