...
SRC_URI = "file://nfsserver"

do_unpack_append() {
	bb.build.exec_func('remove_debian_patch', d)
}

remove_debian_patch() {
	# Default path is /usr/sbin/sm-notify
	sed -i "/01-sm-notify-in-sbin.patch/d" ${S}/debian/patches/series
}
