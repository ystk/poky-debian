...
# Makefile.SH.patch: Required to replace all 'miniperl' by 'hostperl'
# perl-dynloader.patch, letgcc-find-errno.patch: Fix compile errors
# cross-generate_uudmap.patch: Needed when cross-compiling
...
"
FILES_perl-module-cpan += " \
${libdir}/perl/${PERL_PV}/CPAN \
${libdir}/perl/${PERL_PV}/CPAN.pm \
"
FILES_perl-module-cpanplus += " \
${libdir}/perl/${PERL_PV}/CPANPLUS \
${libdir}/perl/${PERL_PV}/CPANPLUS.pm \
"
FILES_perl-module-unicore += "${libdir}/perl/${PERL_PV}/unicore"
python populate_packages_prepend () {
	libdir = bb.data.expand('${libdir}/perl/${PERL_PV}', d)
	do_split_packages(d, libdir, 'auto/(Encode/.[^/]*)/.*', 'perl-module-%s', 'perl module %s', recursive=True, allow_dirs=False, match_path=True, prepend=False)
	do_split_packages(d, libdir, 'auto/([^/]*)/.*', 'perl-module-%s', 'perl module %s', recursive=True, allow_dirs=False, match_path=True, prepend=False)
	do_split_packages(d, libdir, 'Module/([^\/]*).*', 'perl-module-%s', 'perl module %s', recursive=True, allow_dirs=False, match_path=True, prepend=False)
	do_split_packages(d, libdir, '(^(?!(CPAN\/|CPANPLUS\/|Module\/|unicore\/|auto\/)[^\/]).*)\.(pm|pl|e2x)', 'perl-module-%s', 'perl module %s', recursive=True, allow_dirs=False, match_path=True, prepend=False)
}
# Replace PV by PERL_PV (END)

# Replace dynamic version numbers (@hoge@) in config.sh by ${PERL_PV}
do_configure_prepend() {
	VERSION=$(echo ${PERL_PV} | cut -d "." -f 2)
	SUBVERSION=$(echo ${PERL_PV} | cut -d "." -f 3)
	API_VERSION=$VERSION
	API_SUBVERSION=0

	sed -i "s|@VERSION@|$VERSION|" ${WORKDIR}/config.sh
	sed -i "s|@SUBVERSION@|$SUBVERSION|" ${WORKDIR}/config.sh
	sed -i "s|@API_VERSION@|$API_VERSION|" ${WORKDIR}/config.sh
	sed -i "s|@API_SUBVERSION@|$API_SUBVERSION|" ${WORKDIR}/config.sh
}
...
do_compile() {
	...
	# Replace "cpan/" by "ext/"
	sed -i -e 's|/usr/include|${STAGING_INCDIR}|g' ext/Compress-Raw-Zlib/config.in
	sed -i -e 's|/usr/lib|""|g' ext/Compress-Raw-Zlib/config.in

	# Delete "cpan/ExtUtils-MakeMaker"
	sed -i -e 's|SYSROOTLIB|${STAGING_LIBDIR}|g' lib/ExtUtils/Liblist/Kid.pm
...
# perl-base
# This package installs (depends on) only core perl modules.
# These packages are included in "perl-base" of Debian.
ALLOW_EMPTY_perl-base = "1"
PACKAGES_append = " perl-base "
RDEPENDS_perl-base = " \
perl \
perl-lib \
